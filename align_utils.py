...
def get_uroman_tokens(norm_transcripts, uroman_root_dir, iso=None):
    # Écriture des transcriptions dans un fichier temporaire d'entrée
    with tempfile.NamedTemporaryFile(mode="w", delete=False, encoding="utf-8") as tf:
        for t in norm_transcripts:
            tf.write(t + "\n")
        input_filename = tf.name  # Le fichier est fermé à la fin du bloc 'with'

    # Création d'un fichier temporaire de sortie
    # On crée le fichier, puis on le ferme immédiatement pour permettre à Perl d'y écrire
    with tempfile.NamedTemporaryFile(mode="w", delete=False, encoding="utf-8") as tf2:
        output_filename = tf2.name
    # A ce stade, input_filename et output_filename sont bien fermés

    # Vérifier que le script uroman.pl existe
    uroman_script = os.path.join(uroman_root_dir, "uroman.pl")
    assert os.path.exists(uroman_script), f"uroman not found: {uroman_script}"

    # Construction de la commande avec redirections
    cmd = f'perl "{uroman_script}"'
    if iso in special_isos_uroman:
        cmd += f" -l {iso}"
    cmd += f' < "{input_filename}" > "{output_filename}"'
    print("Running:", cmd)
    ...
    # Vérification que le nombre de lignes produit est correct
    assert len(outtexts) == len(norm_transcripts), (
        f"Uroman only produced {len(outtexts)} lines, but we gave it {len(norm_transcripts)} lines! Something went wrong."
    )
